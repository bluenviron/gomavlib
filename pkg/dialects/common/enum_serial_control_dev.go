//autogenerated:yes
//nolint:revive,misspell,govet,lll,dupl,gocritic
package common

import (
	"fmt"
	"strconv"
)

// SERIAL_CONTROL device types
type SERIAL_CONTROL_DEV uint64

const (
	// First telemetry port
	SERIAL_CONTROL_DEV_TELEM1 SERIAL_CONTROL_DEV = 0
	// Second telemetry port
	SERIAL_CONTROL_DEV_TELEM2 SERIAL_CONTROL_DEV = 1
	// First GPS port
	SERIAL_CONTROL_DEV_GPS1 SERIAL_CONTROL_DEV = 2
	// Second GPS port
	SERIAL_CONTROL_DEV_GPS2 SERIAL_CONTROL_DEV = 3
	// Third telemetry port
	SERIAL_CONTROL_DEV_TELEM3 SERIAL_CONTROL_DEV = 4
	// Fourth telemetry port
	SERIAL_CONTROL_DEV_TELEM4 SERIAL_CONTROL_DEV = 5
	// system shell
	SERIAL_CONTROL_DEV_SHELL SERIAL_CONTROL_DEV = 10
	// Electronic Speed Controller 0
	SERIAL_CONTROL_DEV_ESC0 SERIAL_CONTROL_DEV = 20
	// Electronic Speed Controller 1
	SERIAL_CONTROL_DEV_ESC1 SERIAL_CONTROL_DEV = 21
	// Electronic Speed Controller 2
	SERIAL_CONTROL_DEV_ESC2 SERIAL_CONTROL_DEV = 22
	// Electronic Speed Controller 3
	SERIAL_CONTROL_DEV_ESC3 SERIAL_CONTROL_DEV = 23
	// Electronic Speed Controller 4
	SERIAL_CONTROL_DEV_ESC4 SERIAL_CONTROL_DEV = 24
	// Electronic Speed Controller 5
	SERIAL_CONTROL_DEV_ESC5 SERIAL_CONTROL_DEV = 25
	// Electronic Speed Controller 6
	SERIAL_CONTROL_DEV_ESC6 SERIAL_CONTROL_DEV = 26
	// Electronic Speed Controller 7
	SERIAL_CONTROL_DEV_ESC7 SERIAL_CONTROL_DEV = 27
	// SERIAL0
	SERIAL_CONTROL_SERIAL0 SERIAL_CONTROL_DEV = 100
	// SERIAL1
	SERIAL_CONTROL_SERIAL1 SERIAL_CONTROL_DEV = 101
	// SERIAL2
	SERIAL_CONTROL_SERIAL2 SERIAL_CONTROL_DEV = 102
	// SERIAL3
	SERIAL_CONTROL_SERIAL3 SERIAL_CONTROL_DEV = 103
	// SERIAL4
	SERIAL_CONTROL_SERIAL4 SERIAL_CONTROL_DEV = 104
	// SERIAL5
	SERIAL_CONTROL_SERIAL5 SERIAL_CONTROL_DEV = 105
	// SERIAL6
	SERIAL_CONTROL_SERIAL6 SERIAL_CONTROL_DEV = 106
	// SERIAL7
	SERIAL_CONTROL_SERIAL7 SERIAL_CONTROL_DEV = 107
	// SERIAL8
	SERIAL_CONTROL_SERIAL8 SERIAL_CONTROL_DEV = 108
	// SERIAL9
	SERIAL_CONTROL_SERIAL9 SERIAL_CONTROL_DEV = 109
)

var value_to_label_SERIAL_CONTROL_DEV = map[SERIAL_CONTROL_DEV]string{
	SERIAL_CONTROL_DEV_TELEM1: "SERIAL_CONTROL_DEV_TELEM1",
	SERIAL_CONTROL_DEV_TELEM2: "SERIAL_CONTROL_DEV_TELEM2",
	SERIAL_CONTROL_DEV_GPS1:   "SERIAL_CONTROL_DEV_GPS1",
	SERIAL_CONTROL_DEV_GPS2:   "SERIAL_CONTROL_DEV_GPS2",
	SERIAL_CONTROL_DEV_TELEM3: "SERIAL_CONTROL_DEV_TELEM3",
	SERIAL_CONTROL_DEV_TELEM4: "SERIAL_CONTROL_DEV_TELEM4",
	SERIAL_CONTROL_DEV_SHELL:  "SERIAL_CONTROL_DEV_SHELL",
	SERIAL_CONTROL_DEV_ESC0:   "SERIAL_CONTROL_DEV_ESC0",
	SERIAL_CONTROL_DEV_ESC1:   "SERIAL_CONTROL_DEV_ESC1",
	SERIAL_CONTROL_DEV_ESC2:   "SERIAL_CONTROL_DEV_ESC2",
	SERIAL_CONTROL_DEV_ESC3:   "SERIAL_CONTROL_DEV_ESC3",
	SERIAL_CONTROL_DEV_ESC4:   "SERIAL_CONTROL_DEV_ESC4",
	SERIAL_CONTROL_DEV_ESC5:   "SERIAL_CONTROL_DEV_ESC5",
	SERIAL_CONTROL_DEV_ESC6:   "SERIAL_CONTROL_DEV_ESC6",
	SERIAL_CONTROL_DEV_ESC7:   "SERIAL_CONTROL_DEV_ESC7",
	SERIAL_CONTROL_SERIAL0:    "SERIAL_CONTROL_SERIAL0",
	SERIAL_CONTROL_SERIAL1:    "SERIAL_CONTROL_SERIAL1",
	SERIAL_CONTROL_SERIAL2:    "SERIAL_CONTROL_SERIAL2",
	SERIAL_CONTROL_SERIAL3:    "SERIAL_CONTROL_SERIAL3",
	SERIAL_CONTROL_SERIAL4:    "SERIAL_CONTROL_SERIAL4",
	SERIAL_CONTROL_SERIAL5:    "SERIAL_CONTROL_SERIAL5",
	SERIAL_CONTROL_SERIAL6:    "SERIAL_CONTROL_SERIAL6",
	SERIAL_CONTROL_SERIAL7:    "SERIAL_CONTROL_SERIAL7",
	SERIAL_CONTROL_SERIAL8:    "SERIAL_CONTROL_SERIAL8",
	SERIAL_CONTROL_SERIAL9:    "SERIAL_CONTROL_SERIAL9",
}

var label_to_value_SERIAL_CONTROL_DEV = map[string]SERIAL_CONTROL_DEV{
	"SERIAL_CONTROL_DEV_TELEM1": SERIAL_CONTROL_DEV_TELEM1,
	"SERIAL_CONTROL_DEV_TELEM2": SERIAL_CONTROL_DEV_TELEM2,
	"SERIAL_CONTROL_DEV_GPS1":   SERIAL_CONTROL_DEV_GPS1,
	"SERIAL_CONTROL_DEV_GPS2":   SERIAL_CONTROL_DEV_GPS2,
	"SERIAL_CONTROL_DEV_TELEM3": SERIAL_CONTROL_DEV_TELEM3,
	"SERIAL_CONTROL_DEV_TELEM4": SERIAL_CONTROL_DEV_TELEM4,
	"SERIAL_CONTROL_DEV_SHELL":  SERIAL_CONTROL_DEV_SHELL,
	"SERIAL_CONTROL_DEV_ESC0":   SERIAL_CONTROL_DEV_ESC0,
	"SERIAL_CONTROL_DEV_ESC1":   SERIAL_CONTROL_DEV_ESC1,
	"SERIAL_CONTROL_DEV_ESC2":   SERIAL_CONTROL_DEV_ESC2,
	"SERIAL_CONTROL_DEV_ESC3":   SERIAL_CONTROL_DEV_ESC3,
	"SERIAL_CONTROL_DEV_ESC4":   SERIAL_CONTROL_DEV_ESC4,
	"SERIAL_CONTROL_DEV_ESC5":   SERIAL_CONTROL_DEV_ESC5,
	"SERIAL_CONTROL_DEV_ESC6":   SERIAL_CONTROL_DEV_ESC6,
	"SERIAL_CONTROL_DEV_ESC7":   SERIAL_CONTROL_DEV_ESC7,
	"SERIAL_CONTROL_SERIAL0":    SERIAL_CONTROL_SERIAL0,
	"SERIAL_CONTROL_SERIAL1":    SERIAL_CONTROL_SERIAL1,
	"SERIAL_CONTROL_SERIAL2":    SERIAL_CONTROL_SERIAL2,
	"SERIAL_CONTROL_SERIAL3":    SERIAL_CONTROL_SERIAL3,
	"SERIAL_CONTROL_SERIAL4":    SERIAL_CONTROL_SERIAL4,
	"SERIAL_CONTROL_SERIAL5":    SERIAL_CONTROL_SERIAL5,
	"SERIAL_CONTROL_SERIAL6":    SERIAL_CONTROL_SERIAL6,
	"SERIAL_CONTROL_SERIAL7":    SERIAL_CONTROL_SERIAL7,
	"SERIAL_CONTROL_SERIAL8":    SERIAL_CONTROL_SERIAL8,
	"SERIAL_CONTROL_SERIAL9":    SERIAL_CONTROL_SERIAL9,
}

// MarshalText implements the encoding.TextMarshaler interface.
func (e SERIAL_CONTROL_DEV) MarshalText() ([]byte, error) {
	if name, ok := value_to_label_SERIAL_CONTROL_DEV[e]; ok {
		return []byte(name), nil
	}
	return []byte(strconv.Itoa(int(e))), nil
}

// UnmarshalText implements the encoding.TextUnmarshaler interface.
func (e *SERIAL_CONTROL_DEV) UnmarshalText(text []byte) error {
	if value, ok := label_to_value_SERIAL_CONTROL_DEV[string(text)]; ok {
		*e = value
	} else if value, err := strconv.Atoi(string(text)); err == nil {
		*e = SERIAL_CONTROL_DEV(value)
	} else {
		return fmt.Errorf("invalid label '%s'", text)
	}
	return nil
}

// String implements the fmt.Stringer interface.
func (e SERIAL_CONTROL_DEV) String() string {
	val, _ := e.MarshalText()
	return string(val)
}
